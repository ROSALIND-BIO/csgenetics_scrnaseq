...
class CellCaller:
   ...
   def make_pd_plots(self):
      ...
      if self.single_species:
         ...
         plt.title(f"Cell Caller minimum count\nthreshold calculation: {self.sample_name}", fontdict = {'family':'sans-serif','color':'black','size':12,'fontweight':'bold'})
         ...
         plt.axvline(np.log10(self.minimum_count_threshold), color='black', label=f"Default\n{str(round(self.minimum_count_threshold))}")
         plt.title(f"Cell Caller minimum human count threshold calculation\nin majority human-count barcodes: {self.sample_name}", fontdict={'family': 'sans-serif', 'color': 'black', 'size': 12, 'fontweight': 'bold'})
         ...
         plt.axvline(np.log10(self.minimum_count_threshold), color='black', label=f"Default\n{str(round(self.minimum_count_threshold))}")
         plt.title(f"Cell Caller minimum mouse count threshold calculation\nin majority mouse-count barcodes: {self.sample_name}", fontdict={'family': 'sans-serif', 'color': 'black', 'size': 12, 'fontweight': 'bold'})
